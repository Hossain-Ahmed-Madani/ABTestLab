...
(function () {
    ...
    function handleTargetPackSizeClick() {
        const pack_size = TEST_VARIATION === 1 ? "4" : "5";
        const targetInput = document.querySelector(`.bundle-builder__pack-size-options input[type=radio][value="${pack_size}"]`);

        // First remove all checked attributes
        document.querySelectorAll(".bundle-builder__pack-size-options input[type=radio]").forEach((item) => {
        ...
        const targetLabel = document.querySelector(`.bundle-builder__pack-size-options label.btn[for="${pack_size}-pack"]`);
    ...
    function handleGA4Event() {
        document.querySelector(".bundle-builder__pack-size-options").addEventListener("click", (e) => {
    ...
    waitForElement(
        () =>
            !!(
                // document.readyState === 'complete' &&
                (window.location.href.includes("/products/byob-3") && document.querySelector("body:not(.AB-LME56) .bundle-builder__pack-size-options"))
            ),
        () => {
