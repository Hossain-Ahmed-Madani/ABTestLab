...
(async () => {
    ...
    function updateLayout() {
        ...
        if (storedCardIds) {
            picked = storedCardIds.map((id) => cards.find((card) => card.getAttribute("data-id") === id)).filter((card) => card !== undefined); // Filter out cards that don't exist on current page
        } else {
    ...
    function checkForItems() {
        return !!(q(`body:not(.${page_initials}):not(.${page_initials}--v${test_variation})`) && q("#root") && q(".collection__products-container") && document.readyState === "complete");
    }
