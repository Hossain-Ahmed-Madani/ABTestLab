(async () => {
    ...
    function getTotalSaved() {
        ...
        qq(".quick-cart__item:has(s.quick-cart__item-price)").forEach((item) => {
            const originPrice = q(item, "s.quick-cart__item-price")?.textContent ?? "";
            const savedPrice = q(item, "span.quick-cart__item-price")?.textContent ?? "";
            const quantity = +q(item, "input.quantity-input__input")?.value ?? 1;

            if (originPrice && savedPrice) {
                totalSaved += quantity * (convertToNumber(originPrice) - convertToNumber(savedPrice));
            }
        });
    ...
    function createLayout() {
        ...
        if (targetNode && convertToNumber(targetNode.textContent).toFixed(2) !== totalSaved.toFixed(2)) {
    ...
    function checkForItems() {
        return !!(q(`body:not(.${page_initials}):not(${page_initials}--v${test_variation})`) && q(".quick-cart"));
    }
