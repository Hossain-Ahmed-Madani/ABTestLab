(async () => {
  ...
  function init() {
    ...
    waitForElementAsync(() =>
      q(
        ".product-variant__info.product-variant__info--color .product-variant__label",
      ),
    )
      .then(() => {
        q(
          ".product-variant__info.product-variant__info--color .product-variant__label",
        ).innerText = "Farbe wählen:";
      })
      .catch(() => {
        //
      });
    ...
    waitForElementAsync(
      () =>
        !!(
          q(
            ".product-variant__info.product-variant__info--size .product-variant__label",
          ) && q(".product-size.product-size--active")
        ),
    )
      .then(() => {
        const productSizeVariantLabelElement = q(
          ".product-variant__info.product-variant__info--size .product-variant__label",
        );
        ...
        if (activeSizeElement) {
          productSizeVariantLabelElement.insertAdjacentHTML(
            "afterend",
            /* HTML */ `<span class="product-variant__size-name"
              >${activeSizeElement.textContent}</span
            >`,
          );
        }
      })
    ...
    waitForElementAsync(
      () =>
        !!(
          q(".product-name") &&
          q(".product-detail-attribute-row:has(>.price-in-discount)")
        ),
    )
      .then(() => {
        q(".product-name").insertAdjacentElement(
          "afterend",
          q(".product-detail-attribute-row:has(>.price-in-discount)"),
        );
      })
    ...
    waitForElementAsync(
      () =>
        !!(
          q(
            ".product-detail-attribute-row:has(>ul.product-detail-shipping-commitment)",
    ...
    waitForElementAsync(
      () =>
        !!(
          q(
            ".product-description-parameters table.table tbody tr td:first-child",
          ) &&
          qq(
            ".product-description-parameters table.table tbody tr td:first-child",
          ).some((td) =>
            ["Material und Pflege", "Passform", "Produktdetails"].some((txt) =>
              txt.includes(td.textContent.trim()),
            ),
          )
        ),
    )
      .then(() => {
        ...
        qq(".product-description-parameters table.table tbody tr").forEach(
          (tr) => {
            const label = q(tr, "td:first-child").textContent.trim();
            if (
              ["Material und Pflege", "Passform", "Produktdetails"].some(
                (text) => label.includes(text),
              )
            ) {
        ...
        q(
          ".product-detail-attribute-row:has(> ul.product-detail-shipping-commitment)",
        ).insertAdjacentElement("beforebegin", topProductDescriptionBlock);

        q(".collapsible-block").addEventListener("click", (e) => {
          q(
            ".product-description-parameters:not(.product-description-parameters--top)",
  ...
  function checkForItems() {
    return !!(
      ...
      ) && q(".product-detail-content-inner-wrap-sticky")
    );
  }
