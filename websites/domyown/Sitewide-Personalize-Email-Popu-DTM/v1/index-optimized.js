(async function () {
    ...
    async function waitForPromiseOnMutation(predicate, maxCount = 500) {
        let count = 0;
    ...
    function init() {
    	console.log("Sitewide - Personalize Email Popup [DTM]");
        ...
        $("#desktop #newsletter-signup-modal .modal-content-body > .flex")
            .first()
            .prepend(/* HTML */ ` <div class="image-container" style="background-image: url(${imageURL});"></div> `);
        $("#mobile #newsletter-signup-modal .modal-content-body > div")
            .first()
            .prepend(/* HTML */ ` <div class="image-container" style="background-image: url(${imageURL});"></div> `);
        ...
        $("#newsletter-signup-modal form").after(/* HTML */ `
            <div class="benefits-container">
                <div class="benefit-item">
                    <div class="benefit-item-icon">${truck}</div>
                    <div class="benefit-item-text">Fast, Free Shipping</div>
                </div>
                <div class="benefit-item">
                    <div class="benefit-item-icon">${headset}</div>
                    <div class="benefit-item-text">Free Expert Advice</div>
                </div>
            </div>
    ...
    const foundRequiredItems = await waitForElementAsync(checkCallback);
    const foundNewsletterModal = await waitForPromiseOnMutation(() => document.querySelector("#newsletter-signup-modal"));
    if(foundRequiredItems && foundNewsletterModal) {
        init();
    }
})()