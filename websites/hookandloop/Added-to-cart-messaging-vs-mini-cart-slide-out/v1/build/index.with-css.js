...
.AB-MINI-CART #cart-drawer #cartDrawerContent {
  top: 0;
}
...
.AB-MINI-CART #cart-drawer div[x-text="getHookLoopType(item.product_name)"] {
  margin-top: 6px;
  height: 28px;
}
...
.AB-MINI-CART .ab-product-options-container:empty {
  display: none;
}
.AB-MINI-CART
...
.AB-MINI-CART
  .ab-product-options-container
  div[x-show="showOption(option)"][style="display:none;"] {
...
(async () => {
  ...
  const PRODUCT_DATA = {
    ...
    most_purchased: [
      ...
      },
      {
        ...
        sku: "194149",
        image:
          "https://www.hookandloop.com/media/catalog/product/cache/207e23213cf636ccdef205098cf3c8a3/r/o/roll-mil_spec_hook.jpg",
        price: "$63.50",
      },
    ],
  };
  ...
  async function getSliderDataApi() {
    ...
    const unique_products = all_products.filter(
      (product, index, self) =>
        index === self.findIndex((p) => p.id === product.id),
    );
    const carousel_data = unique_products
      .filter(
        (item) =>
          !added_products.some(
  ...
  async function createCarouselElement() {
    ...
    div.innerHTML = /* HTML */ `
      ...
      <button
        class="ab-carousel-btn ab-carousel-btn--next"
