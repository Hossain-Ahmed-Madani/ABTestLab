...
.AB-EXP-299
  .mt-2.bb-4.lg\:ml-auto.xl\:ml-0.py-2[x-data="initSavingOnVolume()"]:has(
    > .hidden
  ) {
  display: none;
}
...
(async () => {
  ...
  function getPricingTableLayout() {
    ...
    unit =
      q(".discount-box ul li .roll-no span[x-text='discount.unit']")
        .textContent || "ROLLS";
    const maxDiscountQuantity =
      +q(".discount-box ul li:last-child .roll-no span[x-text='discount.qty']")
        .textContent ?? 0;
                    ...
                    ${data
                      .map(
                        ({
                          ...
                          <li
                            class="ab-pricing-table__pricing__item ${closestQuantityToApplyDiscount &&
                            quantity === closestQuantityToApplyDiscount
                              ? "ab-pricing-table__pricing__item--active"
                              : ""}"
                          >
                            <div
                              class="ab-pricing-table__pricing__save-percentage"
                            >
                              -${discount}%
                            </div>
                            <a
                              href="${link}"
                              class="ab-pricing-table__pricing__quantity"
                              ><span class="ab-quantity">${quantity}</span>+
                              <span class="ab-unit">${unit}</span></a
                            >
                            <a
                              href="${link}"
                              class="ab-pricing-table__pricing__price"
                      ...
                      .map(
                        ({
                          ...
                          >
                            <a
                              href="${link}"
                              class="ab-pricing-table__pricing__left"
                            >
                              ...
                              <div
                                class="ab-pricing-table__pricing__save-container"
                              >
                                <div class="ab-pricing-table__pricing__save">
                                  You save $${saveAmount}
                                </div>
  ...
  const debouncedFinalPriceAndActiveQuantityUpdate = debounce((e) => {
    ...
    if (
      currentSelectedQuantity >=
      +q(
        "ul.ab-pricing-table__pricing > li.ab-pricing-table__pricing__item:last-child .ab-quantity",
      ).textContent
    ) {
  ...
  const debouncedRecreatePricingTable = debounce((e) => {
    ...
    q(".ab-pricing-table").innerHTML = /* HTML */ `
      ...
      >
        ✦ Larger quantities?
        <a href="/contact" class="ab-contact-us">Contact us</a>.
      </div>
      <ul class="ab-pricing-table__pricing ab-pricing-table__pricing--mobile">
        ${data
          .map(
