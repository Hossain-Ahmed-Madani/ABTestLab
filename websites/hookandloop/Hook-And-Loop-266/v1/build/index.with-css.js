(function () {
  var interval = setInterval(function () {
    if (document.head) {
      ...
      style.innerHTML = `.AB-266 div:has(> .updated-stock-status) p {
  display: none;
}
.AB-266 .ab-stock-status-container {
  display: flex;
...
.AB-266 .ab-stock-status-container--in-stock .ab-stock-status-text {
  color: #008800;
}
.AB-266 .ab-stock-status-container--low-stock {
  cursor: pointer;
}
.AB-266 .ab-stock-status-container--low-stock .ab-stock-status-text {
  color: #ff9903;
}
.AB-266 .ab-stock-status-container--available-to-order {
  cursor: pointer;
}
.AB-266 .ab-stock-status-container--available-to-order .ab-stock-status-text {
  color: #ff9903;
}
.AB-266 .ab-stock-status-container:hover .ab-stock-status-text-tooltip,
.AB-266 .ab-stock-status-container:hover .ab-stock-status-text-tooltip-arrow {
  display: block;
}
.AB-266 .ab-stock-status-icon {
  display: flex;
  justify-content: flex-start;
  align-items: center;
}
.AB-266 .ab-stock-status-icon svg {
  width: 30px;
  height: 30px;
}
.AB-266 .ab-stock-status-text {
...
.AB-266 .ab-stock-status-text-tooltip {
  display: none;
  width: 300px;
...
.AB-266 .ab-stock-status-text-tooltip-text {
  font-family: poppins, sans-serif;
  font-size: 15px;
  line-height: 1.5;
  font-weight: 400;
...
.AB-266 .ab-stock-status-text-tooltip-arrow {
  display: none;
  position: absolute;
  width: 15px;
  height: 15px;
...
@media screen and (max-width: 1200px) {
  .AB-266 .product-info-main.container {
    overflow-x: hidden;
  }
  .AB-266 .ab-stock-status-text-tooltip {
    width: 250px;
...
(async () => {
  ...
  const DATA = {
    ...
    "low stock": {
      ...
      label: "Low stock",
      tooltip_message:
        "Low Stock: Available to order, but supply may be limited. Larger quantities may take additional time to fulfill.",
    },
    "available to order": {
      icon: /* HTML */ `
        <svg
      ...
      tooltip_message:
        "Available to Order: This item may be ordered now, but it is not currently available for immediate shipment. Fulfillment timing may vary.",
    },
  };
  ...
  function createLayout(currentStatus) {
    ...
    targetNodes.forEach((targetNode) => {
      targetNode.insertAdjacentHTML(
        ...
        /* HTML */ `
          <div
            class="ab-stock-status-container ab-stock-status-container--${currentStatus
              .split(" ")
              .join("-")}"
          >
  ...
  function customEventListener() {
    window.addEventListener("configurable-selection-changed", (e) => {
      const currentStatus =
        e?.detail?.sstockMessage.toLowerCase().trim() ?? null;
  ...
  function init() {
    ...
    const currentStatus = q(".updated-stock-status p")
