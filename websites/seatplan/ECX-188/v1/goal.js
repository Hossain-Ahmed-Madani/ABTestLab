(async () => {
    const TEST_CONFIG = {
        ...
        test_name: "[ECX - 188] Production - Seat View Messaging | GLOBl",
        page_initials: "GLOBAL-EXP-188",
        test_variation: 1,
        test_version: 0.0001,
    };
    ...
    function init() {
        ...
        q(".ticket-calendar").addEventListener("click", (e) => {
            console.log("GOAL: Interact with calendar | Production Page | JS");
            window._conv_q = window._conv_q || [];
            _conv_q.push(["triggerConversion", "1004110604"]);
        });
    }

    function checkForItems() {
        return !!(q(`body:not(.${page_initials}):not(.${page_initials}--v${test_variation})`) && q(".ticket-calendar"));
    }
