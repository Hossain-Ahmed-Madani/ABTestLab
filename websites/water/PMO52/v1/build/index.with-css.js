(function () {
  var interval = setInterval(function () {
    if (document.head) {
      ...
      style.innerHTML = `.AB-PMO52 .cart__banner + .ab-social-proof-badge {
  margin-left: 4px;
  margin-top: 18px;
  margin-bottom: 5px;
}
.AB-PMO52
  .summit-Title-root.summary-head.cart__section-title
  + .ab-social-proof-badge {
  display: none;
}
  ...
  async function INIT_PMO32() {
    try {
      ...
      const TXT = [
        ...
        "Delivered to 8,200+ homes this week",
      ];

      qq(
        ".cart__banner, .summit-Title-root.summary-head.cart__section-title",
      ).forEach((item) => {
        item.insertAdjacentHTML(
          ...
          /* HTML */ `
            <div class="ab-social-proof-badge">
              ...
              <div class="ab-social-proof-badge__txt">
                ${TXT[test_variation - 1]}
              </div>
            </div>
          `,
        );
